...
<body>
	...
	ArrayList<Donor> donorList;%>
				<%
					...
					donorList = (ArrayList) request.getAttribute("donorList");
					if (donorList != null && donorList.size() > 0) {
						for (int size = 1; size <= donorList.size(); size++) {
							dummy_donor = new Donor();
							dummy_donor = (Donor) donorList.get(size - 1);
							System.out.println(dummy_donor.getName());
				%>
				<tr>
					<th scope="row"><%=dummy_donor.getName()%></th>
					<td><%=dummy_donor.getContact()%></td>
					<td><%=dummy_donor.getAddress()%></td>
					<td><%=dummy_donor.getBlood_group()%></td>
					<td><%=dummy_donor.getStatus()%></td>
					<%
						}
						}
					%>
