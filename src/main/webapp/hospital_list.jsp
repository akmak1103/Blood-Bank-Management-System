...
<body>
	...
	<div class="container">
		<table class="table table-striped" width="900" border="0"
			...
			<tbody>
				<%!Hospital dummy_hospital;
	ArrayList<Hospital> hospitalList;%>
				<%
					hospitalList = new ArrayList();
					hospitalList = (ArrayList) request.getAttribute("hospitalList");
					if (hospitalList != null && hospitalList.size() > 0) {
						for (int size = 1; size <= hospitalList.size(); size++) {
							dummy_hospital = new Hospital();
							dummy_hospital = (Hospital) hospitalList.get(size - 1);
							System.out.println(dummy_hospital.getName());
				%>
				<tr>
					<th scope="row"><%=dummy_hospital.getId()%></th>
					<td><%=dummy_hospital.getName()%></td>
					<td><%=dummy_hospital.getAddress()%></td>
					<td><%=dummy_hospital.getBlood_bank_id()%></td>
					<%
						}
						}
					%>
